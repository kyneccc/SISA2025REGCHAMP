...
openstack subnet create --no-dhcp --subnet-range 10.0.0.0/24 --gateway 10.0.0.6  --network  External-net  exsub --insecure
openstack network create Internal-net --insecure
openstack subnet create  --no-dhcp --subnet-range 192.168.10.0/26 --gateway 192.168.10.62  --network Internal-net  insub --insecure
...
openstack port create --network Management-net --fixed-ip ip-address=192.168.10.71 hap1mg --insecure
...
openstack floating ip create --port lbex Inet  --insecure
